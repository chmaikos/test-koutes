...
περισσότερες από μία αποθήκες. Το πεδίο περιγραφής
...
1. επιβεβαιώστε κάθε Lot και Παλέτα,
...
4. για μετακινήσεις ελέγξτε την προέλευση, τη διαδρομή `Source → Target`, την
   τρέχουσα Παλέτα και την Παλέτα προορισμού,
...
- και η επιλεγμένη Παλέτα είναι ενεργή και στο ίδιο Lot. Επιτρέπεται να είναι η
  ίδια οργανωτική Παλέτα που είχε ήδη το Κιβώτιο στην αποθήκη προέλευσης.
...
Παλέτες που λείπουν, είναι αρχειοθετημένες ή ανήκουν σε άλλο Lot. Οι
δεσμεύσεις δεν ακυρώνονται εξαναγκασμένα από αυτή τη ροή.

Κατά την ολοκλήρωση, η εφαρμογή επιλύει ή δημιουργεί την ενεργή Παλέτα στο
σωστό Lot, χωρίς να της αποδίδει αποθήκη. Κάθε γραμμή αιτήματος κρατά αμετάβλητα
`pallet_id` και εμφανιζόμενο snapshot Παλέτας, όπως ακριβώς κρατά το Lot, ώστε
μελλοντική μετονομασία ή απορρόφηση να μην ξαναγράφει το ιστορικό.
...
- Κάθε νέα γραμμή πρέπει να έχει ενεργή Παλέτα στο ίδιο Lot· η αποθήκη ανήκει
  στο Κιβώτιο και η ένδειξη Unassigned αφορά μόνο legacy δεδομένα.
...
τα αναθέσει σε ενεργή Παλέτα ίδιου Lot, εφόσον έχει πρόσβαση στην αποθήκη του
Κιβωτίου. Νέα παραλαβή χωρίς Παλέτα
