...
Έκδοση οδηγού: 19 Αυγούστου 2026
...
Η φυσική ιεραρχία είναι **Αποθήκη → Lot → Παλέτα → Κιβώτιο**. Το Lot εκφράζει
την παρτίδα, η Παλέτα την πραγματική ομάδα κιβωτίων μέσα σε συγκεκριμένη
αποθήκη και το Κιβώτιο τη μονάδα που αλλάζει κατάσταση. Το πεδίο περιγραφής
...
- επιλέξετε **Unassigned** για παλαιά κιβώτια χωρίς παλέτα,
...
4. Συμπληρώστε:
   - **Box number**,
   - **Lot**,
   - υποχρεωτικά **Pallet** (επιλογή υπάρχουσας ή δημιουργία νέας),
...
6. Αντιστοιχίστε τις στήλες του αρχείου με:
   ...
   - υποχρεωτικά παλέτα από στήλη ή σταθερή τιμή για όλες τις γραμμές,
...
- Πολλαπλές γραμμές με ίδιο lot και ίδιο box number συγχωνεύονται σε ένα
  φυσικό κιβώτιο μόνο όταν συμφωνεί και η κανονικοποιημένη παλέτα. Διαφορετικές
  παλέτες για το ίδιο φυσικό κιβώτιο είναι σύγκρουση και η εισαγωγή σταματά.
...
Κάθε ενεργή Παλέτα έχει μία τρέχουσα αποθήκη.

Από το **Pallets** μπορείτε να:

- αναζητήσετε με αριθμό Παλέτας, όνομα Lot ή όνομα αποθήκης,
- φιλτράρετε ανά Lot, αποθήκη, πρόοδο και ενεργή/αρχειοθετημένη κατάσταση,
...
- δείτε πλήθη/καταστάσεις και ποσοστό ολοκλήρωσης με τους ίδιους κανόνες του
  Lot,
- ανοίξετε λεπτομέρεια με σελιδοποιημένη λίστα Κιβωτίων και πλήρες Activity.

Viewer και Warehouse Mover έχουν ανάγνωση μόνο στις αποθήκες τους. Operator και
Admin μπορούν να δημιουργήσουν Παλέτα, να αναθέσουν/αποσυνδέσουν Κιβώτια και να
μετακινήσουν ολόκληρη Παλέτα. Μόνο Admin βλέπει αρχειοθετημένες Παλέτες και
μπορεί να μετονομάσει, αρχειοθετήσει ή επαναφέρει.

#### Επιλογή ή δημιουργία Παλέτας

Ο επιλογέας Παλέτας περιορίζεται πάντοτε στο επιλεγμένο Lot και στην αποθήκη.
Αν αλλάξει οποιοδήποτε από τα δύο, η παλιά επιλογή και η αναζήτηση μηδενίζονται
ώστε να μη σταλεί Παλέτα λάθος πεδίου. Πληκτρολογήστε μέρος του αριθμού και
επιλέξτε αποτέλεσμα. Αν δεν υπάρχει ακριβής κανονικοποιημένη αντιστοίχιση και
έχετε δικαίωμα εγγραφής, εμφανίζεται **Create new pallet**. Αποτυχία λόγω
ταυτόχρονης δημιουργίας/διπλοτύπου εμφανίζει σύγκρουση· ανανεώστε και επιλέξτε
την ήδη υπάρχουσα εγγραφή.

Η Παλέτα είναι υποχρεωτική για κάθε νέα χειροκίνητη ή XLSX παραλαβή, staged
receipt, απευθείας εισερχόμενη εγγραφή και ολοκλήρωση εισερχόμενου αιτήματος.
Παλαιά δεδομένα πριν από τη μετάβαση μπορεί νόμιμα να μην έχουν Παλέτα και
εμφανίζονται ως **Unassigned**. Η συμβατότητα αυτή δεν επιτρέπει νέες
Unassigned εγγραφές.

#### Ανάθεση, αποσύνδεση και μετακίνηση
...
Κιβώτιο και η Παλέτα πρέπει να ανήκουν στο ίδιο Lot και να βρίσκονται στην ίδια
αποθήκη. Η εφαρμογή κλειδώνει και επανελέγχει τα σχετικά δεδομένα πριν τη
μεταβολή· αν άλλαξαν ταυτόχρονα, δεν εφαρμόζει μερικό αποτέλεσμα.

Το **Move pallet** μετακινεί ατομικά την Παλέτα και όλα τα Κιβώτιά της σε ενεργή
αποθήκη. Δεν επιτρέπεται να μείνει Κιβώτιο της Παλέτας στην παλιά αποθήκη.
Ενεργές επιστροφές που συγκρούονται ακυρώνονται με audit και τα IDs τους
εμφανίζονται στο αποτέλεσμα. Η μεμονωμένη μετακίνηση ή επιστροφή Κιβωτίου
διατηρεί επίσης τον κανόνα ίδιας αποθήκης: όπου απαιτείται, η ανάθεση Παλέτας
αφαιρείται ή πρέπει να επιλεγεί συμβατή Παλέτα προορισμού.
...
για κενή Παλέτα και απαιτεί λόγο/τρέχουσα έκδοση. Η επαναφορά απορρίπτεται αν
υπάρχει ενεργή Παλέτα ίδιου αριθμού στο Lot ή αν η αποθήκη είναι ανενεργή.

#### Συγχώνευση Lot, απορρόφηση και purge

Στη συγχώνευση δύο Lots, Παλέτες χωρίς σύγκρουση αριθμού μεταφέρονται στο Lot
στόχο. Αν υπάρχει ίδιος κανονικοποιημένος αριθμός, επιβιώνει η Παλέτα του Lot
στόχου, δέχεται τα Κιβώτια και η Παλέτα πηγής γίνεται αρχειοθετημένο
...
συναλλαγή και η Παλέτα στόχου παραμένει ανέπαφη.
...
Παλετών. Ενεργή Παλέτα — ακόμη και κενή — μπλοκάρει επίσης την αρχειοθέτηση
ολόκληρης αποθήκης.
...
- χειροκίνητα, προσθέτοντας γραμμές lot/pallet/box number/item descriptions,
- από XLSX μέσω προεπισκόπησης και αντιστοίχισης στηλών.

Στην εισαγωγή XLSX:

1. επιλέξτε αρχείο και φύλλο,
2. αντιστοιχίστε υποχρεωτικά lot, pallet και box number, και προαιρετικά item
   descriptions,
3. όλες οι γραμμές είναι αρχικά ενεργές,
4. εξαιρέστε επικεφαλίδες και άσχετες γραμμές,
5. οι επαναλαμβανόμενες γραμμές ίδιου κιβωτίου συγχωνεύονται μόνο όταν
   συμφωνεί η Παλέτα· διαφορετικές Παλέτες εμφανίζονται ως σύγκρουση,
6. ελέγξτε την πραγματική ποσότητα πριν την ολοκλήρωση.

Κατά την ολοκλήρωση, η εφαρμογή επιλύει ή δημιουργεί την ενεργή Παλέτα στο
σωστό Lot και στην αποθήκη παραλαβής. Κάθε γραμμή αιτήματος κρατά αμετάβλητα
`pallet_id` και εμφανιζόμενο snapshot Παλέτας, όπως ακριβώς κρατά το Lot, ώστε
μελλοντική μετονομασία, μετακίνηση ή απορρόφηση να μην ξαναγράφει το ιστορικό.
...
7. Τα αποτελέσματα ομαδοποιούνται ανά Παλέτα για φυσική συλλογή. Παλαιά
   κιβώτια χωρίς Παλέτα εμφανίζονται στην ομάδα **Unassigned**.
...
7. Τα επιλεγμένα κιβώτια μετακινούνται σε μία ατομική συναλλαγή από τη source
   ...
   Η παλιά ανάθεση Παλέτας αποσυνδέεται όταν δεν είναι συμβατή με την target,
   έτσι κανένα Κιβώτιο δεν παραμένει συνδεδεμένο με Παλέτα άλλης αποθήκης.
...
**Pallets**. Περιλαμβάνουν Pallet ID/αριθμό, Lot, τρέχουσα αποθήκη, κατάσταση,
κατανομή Κιβωτίων, επιλέξιμα/ολοκληρωμένα πλήθη, ποσοστό ολοκλήρωσης, έκδοση και
τελευταία δραστηριότητα. Οι μη Admin λαμβάνουν μόνο το ACL-visible σύνολο.
...
- περιέχει ενεργή Παλέτα, ακόμη και αν είναι κενή,
- χρησιμοποιείται ως source ή target σε draft ή ανοικτό αίτημα.

Αν εμφανιστεί σύγκρουση, το μήνυμα αναφέρει πόσα ενεργά κιβώτια, Παλέτες και
ανοικτά αιτήματα πρέπει να διευθετηθούν, καθώς και πόσα από τα αιτήματα
...
- Κάθε νέα γραμμή πρέπει να έχει Παλέτα στο ίδιο Lot και στην αποθήκη
  παραλαβής· η ένδειξη Unassigned αφορά μόνο legacy δεδομένα.
- Αν ίδιο lot/box εμφανίζεται σε περισσότερες γραμμές, όλες πρέπει να δίνουν
  την ίδια κανονικοποιημένη Παλέτα.
...
- Επιλέξτε πρώτα σωστό Lot και αποθήκη· ο picker δείχνει μόνο αυτό το scope.
...
Το **Unassigned** είναι αναμενόμενη ένδειξη μόνο για Κιβώτια που υπήρχαν πριν
από τη λειτουργία Παλετών. Δεν είναι σφάλμα ανάγνωσης. Operator/Admin μπορεί να
τα αναθέσει σε ενεργή Παλέτα ίδιου Lot και αποθήκης. Νέα παραλαβή χωρίς Παλέτα
πρέπει να διορθωθεί στη φόρμα και δεν αποθηκεύεται.
...
Μετακινήστε, επιστρέψτε ή αρχειοθετήστε τα ενεργά κιβώτια, μετακινήστε ή
αρχειοθετήστε όλες τις ενεργές Παλέτες και κλείστε όλα τα ανοικτά αιτήματα.
Δεν μπορεί να αρχειοθετηθεί η τελευταία ενεργή αποθήκη.
...
- Επιβεβαιώστε Lot, Παλέτα και αριθμό Κιβωτίου σε κάθε γραμμή.
...
- Συμπληρώνετε Lot, Παλέτα και ενιαία περιγραφή ειδών με συνέπεια.
- Μετακινείτε ολόκληρη Παλέτα όταν μεταφέρεται όλη η φυσική ομάδα.
