...
Έκδοση οδηγού: 25 Σεπτεμβρίου 2026
...
Η πλήρης ιεραρχία αποθέματος είναι **Lot → Παλέτα → Κιβώτιο → File**. Το Lot
εκφράζει την παρτίδα, η Παλέτα μια οργανωτική ομάδα Κιβωτίων του ίδιου Lot, το
Κιβώτιο τη μονάδα που έχει πραγματική θέση και κατάσταση, και το **File** τον
ξεχωριστό φυσικό φάκελο/αρχείο που βρίσκεται μέσα σε συγκεκριμένο Κιβώτιο. Η
ίδια Παλέτα μπορεί να περιλαμβάνει ταυτόχρονα Κιβώτια σε περισσότερες από μία
αποθήκες. Η ανάθεση Παλέτας είναι προαιρετική: Κιβώτιο χωρίς ανάθεση
εμφανίζεται ως **Unassigned**, χωρίς να δημιουργείται εικονική Παλέτα.

Κάθε φυσικό File έχει υποχρεωτικό reference και προαιρετική περιγραφή. Το
σύστημα εκδίδει αυτόματα αμετάβλητο barcode σε κάθε Lot, Παλέτα, Κιβώτιο και
File· ο χρήστης δεν το πληκτρολογεί ούτε το αλλάζει. Το File είναι
παρακολουθούμενη εγγραφή αποθέματος με θέση μέσα στο Κιβώτιο,
ιστορικό και δυνατότητα μετακίνησης/αρχειοθέτησης. **Δεν είναι ψηφιακό αρχείο
που μεταφορτώνεται.** Τα PDF/JPEG/PNG παραστατικά ERP και τα υποστηρικτικά
attachments ενός αιτήματος είναι ιδιωτικά ψηφιακά αντικείμενα στο RustFS και
περιγράφονται ξεχωριστά στις ενότητες 7.7 και 7.9.

Το παλαιό πεδίο **Item descriptions/Contents** παραμένει μόνο για συμβατότητα
και συνοπτική εμφάνιση παλαιών δεδομένων. Δεν είναι πλέον η αυθεντική λίστα
φυσικών Files. Η migration δημιουργεί ξεχωριστές εγγραφές File από τα παλαιά
τμήματα Contents, χωρίς να θεωρεί ποσότητες ή ελεύθερο κείμενο ως υποκατάστατο
της νέας ταυτότητας File.
...
| --- | --- | --- | --- | --- | --- |
| **Viewer** | Ναι | Μόνο ανάγνωση Κιβωτίων και Files | Δημιουργία και προβολή δικών του/επιτρεπόμενων αιτημάτων | Προβολή | Όχι |
| **Operator** | Ναι | Δημιουργία, επεξεργασία, μετακίνηση και αρχειοθέτηση Files· διαχείριση Κιβωτίων | Δημιουργία και παραλαβή εισερχόμενου αιτήματος όταν είναι ο αιτών | Καταχώριση ημερήσιων στοιχείων | Όχι |
| **Warehouse Mover** | Ναι | Μόνο ανάγνωση Κιβωτίων και Files | Έγκριση/απόρριψη, προετοιμασία, παραστατικά ERP, μεταφορά, άφιξη, εξαιρέσεις και επιβεβαίωση επιστροφής | Προβολή | Όχι |
| **Admin** | Ναι | Πλήρης διαχείριση, επαναφορά Files, ελεγχόμενες παρακάμψεις και ασφαλής μόνιμη εκκαθάριση lot | Πλήρης διαχείριση | Πλήρης διαχείριση | Ναι |
...
- **Files** — λίστα και λεπτομέρειες των φυσικών Files μέσα στα Κιβώτια,
...
Το **Scan barcode** στο επάνω μέρος δέχεται σάρωση από scanner πληκτρολογίου.
Με Enter η εφαρμογή ελέγχει τον κωδικό και ανοίγει απευθείας το Lot, την
Παλέτα, το Κιβώτιο ή το File. Μη έγκυρο check digit, άγνωστος κωδικός,
αρχειοθετημένη/συγχωνευμένη εγγραφή και κωδικός χωρίς πρόσβαση εμφανίζονται
ως ξεχωριστά μηνύματα. Οι οριστικά διαγραμμένες εγγραφές εμφανίζονται μόνο σε
Admin ως ιστορικό tombstone.
...
- αναζητήσετε με αριθμό Κιβωτίου, barcode Κιβωτίου, Lot, Παλέτα,
  compatibility Contents ή reference/description/barcode ενεργού φυσικού File,
...
4. Συμπληρώστε:
   ...
   - προαιρετικά **Pallet** (επιλογή υπάρχουσας, δημιουργία νέας ή **Leave
     Unassigned**),
   - τουλάχιστον ένα φυσικό **File reference**,
   - προαιρετικά **File description** για κάθε File.
...
Το File reference καθαρίζεται από περιττά κενά και πρέπει να είναι μοναδικό
χωρίς διάκριση πεζών/κεφαλαίων μέσα σε ολόκληρο το Lot, όχι μόνο μέσα στο
Κιβώτιο. Το barcode κάθε νέου Lot, Παλέτας, Κιβωτίου και File εκδίδεται
αυτόματα και δεν ζητείται στη φόρμα.
...
6. Αντιστοιχίστε τις στήλες του αρχείου με:
   ...
   - προαιρετικά παλέτα από στήλη ή επιλέξτε ρητά **Leave new boxes
     Unassigned**,
   - υποχρεωτικό **File reference**,
   - προαιρετικά **File description**,
   - προαιρετικά το παλαιό **Legacy contents** μόνο για συμβατότητα.
...
- Κάθε επιλεγμένη γραμμή πρέπει να έχει File reference. Επαναλαμβανόμενες
  γραμμές ίδιου Κιβωτίου γίνονται πολλαπλά Files και συγχωνεύονται σε μία
  εγγραφή Κιβωτίου.
- Το ίδιο File reference δεν μπορεί να δείχνει σε δύο διαφορετικά Κιβώτια του
  ίδιου Lot. Ίδιο reference με διαφορετική περιγραφή είναι σύγκρουση.
- Η περιγραφή File είναι προαιρετική έως 10.000 χαρακτήρες. Το παλαιό Contents
  παραμένει προαιρετικό έως 2.000 χαρακτήρες και δεν αντικαθιστά το File
  reference. Αποθηκευμένα mappings που περιέχουν παλαιά στήλη barcode
  χρειάζονται επανεξέταση και αποθήκευση χωρίς αυτήν.
...
- Lot, Παλέτα/Unassigned, τρέχουσα αποθήκη και πλήθος ενεργών/
  αρχειοθετημένων Files,
- την αυθεντική λίστα φυσικών Files του Κιβωτίου και συνδέσμους στις
  λεπτομέρειές τους,
...
- δείτε πλήθη Κιβωτίων και ενεργών/αρχειοθετημένων Files, καταστάσεις και
  ποσοστό ολοκλήρωσης με τους ίδιους κανόνες του Lot,
...
### 6.12 Φυσικά Files: λίστα, λεπτομέρεια και κύκλος ζωής

Το **File** είναι η μικρότερη παρακολουθούμενη φυσική μονάδα της ιεραρχίας.
Ανήκει υποχρεωτικά σε ένα Κιβώτιο και στο ίδιο Lot με αυτό. Δεν έχει δική του
αποθήκη, Παλέτα ή κατάσταση: κληρονομεί σε πραγματικό χρόνο το Κιβώτιο, την
Παλέτα/ένδειξη Unassigned, την αποθήκη και το Box status. Όταν μετακινείται ή
αλλάζει κατάσταση το Κιβώτιο, γράφεται αντίστοιχο inherited συμβάν στο
ιστορικό κάθε File που περιέχει.

#### Λίστα και αναζήτηση Files

Από το **Files** μπορείτε να:

- αναζητήσετε reference, περιγραφή, barcode File, αριθμό Κιβωτίου, Lot,
  Παλέτα ή αποθήκη,
- φιλτράρετε ανά αποθήκη, Lot, Παλέτα, Κιβώτιο, inherited Box status και
  ενεργό/αρχειοθετημένο File,
- ταξινομήσετε ανά reference, Lot, Παλέτα, Κιβώτιο, αποθήκη, κατάσταση, θέση ή
  ημερομηνία,
- ανοίξετε απευθείας το σχετικό Lot, την Παλέτα, το Κιβώτιο ή τη λεπτομέρεια
  File.

Οι μη Admin βλέπουν μόνο Files των αποθηκών που τους έχουν ανατεθεί. Μόνο Admin
μπορεί να ενεργοποιήσει **Include inherited archived hierarchy** για Files που
κληρονομούν ανενεργό Κιβώτιο/Lot/Παλέτα/αποθήκη. Η επιλογή
ενεργών/αρχειοθετημένων Files είναι ανεξάρτητη από αυτή την κληρονομημένη
κατάσταση.

#### Ταυτότητα και λεπτομέρεια

Το reference είναι υποχρεωτικό, έως 255 χαρακτήρες, καθαρίζεται με trim και
σύμπτυξη εσωτερικών κενών και είναι μοναδικό χωρίς διάκριση πεζών/κεφαλαίων
**μέσα στο Lot**. Για παράδειγμα, τα `ABC 01`, `abc 01` και ` ABC   01 `
είναι η ίδια ταυτότητα στο ίδιο Lot, ακόμη και αν βρίσκονται σε διαφορετικά
Κιβώτια. Η ίδια τιμή επιτρέπεται σε άλλο Lot. Η περιγραφή είναι προαιρετική έως
10.000 χαρακτήρες. Το barcode εκδίδεται αυτόματα, είναι μοναδικό σε όλο το
σύστημα και δεν αλλάζει.

Η λεπτομέρεια δείχνει τη διαδρομή **Lot → Pallet/Unassigned → Box → File**,
reference, περιγραφή, barcode Code 128, θέση μέσα στο Κιβώτιο, inherited
αποθήκη και status, έκδοση και χρόνους δημιουργίας/ενημέρωσης. Το **Print
label** εκτυπώνει μόνο την ετικέτα. Το **File audit timeline**
κρατά δημιουργία, αλλαγές, μετακίνηση σε άλλο Κιβώτιο, αρχειοθέτηση,
επαναφορά, αλλαγή Lot του Κιβωτίου και κληρονομημένες αλλαγές θέσης/status.
Ιστορικά στοιχεία από αποθήκη που δεν είναι πλέον προσβάσιμη αποκρύπτονται από
μη Admin.

#### Δημιουργία, αλλαγή και μετακίνηση

Operator και Admin με πρόσβαση στην τρέχουσα αποθήκη μπορούν να δημιουργούν
File σε ενεργό Κιβώτιο ενεργού Lot και να αλλάζουν reference ή περιγραφή. Το
barcode δεν επεξεργάζεται. Κάθε μεταβολή χρησιμοποιεί την τρέχουσα έκδοση· αν
μεσολάβησε άλλη αλλαγή, ανανεώστε τη σελίδα και ελέγξτε ξανά.

1. Για δημιουργία, ανοίξτε τη λεπτομέρεια Κιβωτίου και στην ενότητα
   **Contained physical Files** πατήστε **Add File**.
2. Συμπληρώστε υποχρεωτικό **Reference** και προαιρετικά **Description**,
   έπειτα **Save File**. Το barcode εμφανίζεται μετά την αποθήκευση.
3. Για αλλαγή, πατήστε **Edit** στη γραμμή του Κιβωτίου ή στη λεπτομέρεια
   File, ελέγξτε την τρέχουσα έκδοση και αποθηκεύστε.

Με **Move** επιλέγετε άλλο ενεργό Κιβώτιο **του ίδιου Lot**. Ο χρήστης πρέπει
να έχει πρόσβαση τόσο στην αποθήκη προέλευσης όσο και στην αποθήκη προορισμού.
Η εφαρμογή επαναριθμεί με ασφάλεια τις θέσεις στα δύο Κιβώτια. Δεν επιτρέπεται
μετακίνηση File σε διαφορετικό Lot· για τέτοια φυσική διόρθωση πρέπει πρώτα να
επιβεβαιωθεί η σωστή επιχειρησιακή ταυτότητα, όχι να παρακαμφθεί ο κανόνας.

#### Αρχειοθέτηση, επαναφορά και ενεργές επιστροφές

Η αρχειοθέτηση File απαιτεί αιτιολογία και αφαιρεί μόνο το File από το ενεργό
περιεχόμενο· το Κιβώτιο και η ροή του δεν αλλάζουν. Η επαναφορά από την
οθόνη γίνεται από Admin, επίσης με αιτιολογία, και τοποθετεί το File ξανά σε
έγκυρη θέση. Τα archived Files παραμένουν στο ιστορικό και στις αντίστοιχες
αναφορές.

Αν το Κιβώτιο δεσμεύεται από ενεργό αίτημα επιστροφής, επιτρέπεται αλλαγή μόνο
της description, επειδή το request snapshot και το barcode δεν ξαναγράφονται.
Αλλαγή reference, μετακίνηση, αρχειοθέτηση ή επαναφορά μπλοκάρεται. Μόνο Admin
μπορεί να χρησιμοποιήσει ρητό **Force**, με υποχρεωτική αιτιολογία· τότε οι
επηρεαζόμενες ενεργές δεσμεύσεις επιστροφής ακυρώνονται και καταγράφονται στο
audit. Δεν υπάρχει σιωπηρή ακύρωση.

#### Πλήθη στη λοιπή εφαρμογή

Οι λίστες/λεπτομέρειες **Boxes**, **Lots** και **Pallets** δείχνουν χωριστά
ενεργά και αρχειοθετημένα Files. Τα Lot/Pallet πλήθη σέβονται το ίδιο ACL scope
με τα ορατά Κιβώτια. Οι υποψήφιες επιστροφές δείχνουν το πλήθος ενεργών Files
και σύντομη σύνοψη references, ώστε η επιλογή να γίνεται με φυσικό έλεγχο.
...
- χειροκίνητα, προσθέτοντας γραμμές lot/προαιρετική pallet/box number και ένα
  ή περισσότερα φυσικά Files με υποχρεωτικό reference και προαιρετική
  description,
...
2. αντιστοιχίστε υποχρεωτικά lot, box number και File reference, προαιρετικά
   File description, και για pallet επιλέξτε στήλη ή ρητά **Leave new boxes
   Unassigned**,
...
3. ελέγξτε χωριστά τα **New boxes**, τα **Existing boxes to relocate**, τα
   **Blocked rows** και τη συμφωνία Files: **new**, **update**, **move**,
   **preserve** ή **blocked**,
...
5. αν υπάρχουν επιλέξιμες μετακινήσεις Κιβωτίων, ενεργοποιήστε τη μία κοινή αποδοχή
   **Move all … eligible matched boxes**· είναι αρχικά ανενεργή και δεν υπάρχει
   επιλογή ανά γραμμή,
6. αν οποιοδήποτε File reference είναι ενεργό σε άλλο Κιβώτιο του ίδιου Lot,
   ελέγξτε source Box/warehouse/status και ενεργοποιήστε ξεχωριστά το αρχικά
   ανενεργό **Move all … matched files**· η αποδοχή μετακίνησης Κιβωτίων δεν
   υποκαθιστά την αποδοχή μετακίνησης Files,
7. πατήστε **Complete … boxes** για να αποδεχθείτε μαζί όλα τα νέα και όλα τα
   εγκεκριμένα μετακινούμενα κιβώτια.

Η συμφωνία File είναι μη καταστροφική:

- άγνωστο reference δημιουργείται στο mapped Κιβώτιο,
- γνωστό reference στο ίδιο Κιβώτιο ενημερώνει μόνο όταν αλλάζει η
  description· το barcode παραμένει το ίδιο,
- ενεργό reference που δεν περιλαμβάνεται στο εισερχόμενο mapping
  **διατηρείται** και δεν διαγράφεται,
- γνωστό ενεργό reference σε άλλο Κιβώτιο του ίδιου Lot προτείνεται ως
  μετακίνηση και απαιτεί τη χωριστή αποδοχή,
- archived reference, μη διαθέσιμο/αρχειοθετημένο source Box, διαφορετικό Lot
  ή source Box με ενεργή κράτηση επιστροφής μπλοκάρει τη γραμμή.
...
προορισμό και παλιά/νέα Παλέτα. Κενό παλαιό mapped Contents δεν διαγράφει το
υπάρχον. Τα δομημένα Files συμφιλιώνονται με τους παραπάνω κανόνες και οι
αλλαγές γράφονται στο File audit. Αν η Παλέτα
...
ξανά τη συνολική αποδοχή μετακίνησης. Η δημιουργία/μετακίνηση Κιβωτίων, η
δημιουργία/ενημέρωση/μετακίνηση/διατήρηση Files, οι Παλέτες, τα compatibility
Contents, τα request snapshots, οι γραμμές και η ολοκλήρωση του αιτήματος
αποθηκεύονται ως μία ατομική ενέργεια.
...
8. Κάθε υποψήφιο Κιβώτιο δείχνει πλήθος ενεργών Files και σύντομη σύνοψη
   references. Η επιστροφή παραμένει **box-based**: επιλέγετε ολόκληρα
   Κιβώτια, όχι μεμονωμένα Files.
9. Όλα τα επιλέξιμα κιβώτια επιλέγονται αρχικά.
10. Αποεπιλέξτε όσα θα παραμείνουν για μεταγενέστερη μερική επιστροφή.
11. Ελέγξτε τη διαδρομή `Source → Target` και πατήστε **Submit request**.
...
Κατά τη δημιουργία του αιτήματος αποθηκεύεται για κάθε γραμμή αμετάβλητο
snapshot όλων των τότε ενεργών Files: reference, description, barcode, θέση
και σύνδεση με το tracked File. Μεταγενέστερη αλλαγή metadata ή θέσης File δεν
ξαναγράφει το αίτημα. Έτσι το request detail αποδεικνύει τι περιείχε το
Κιβώτιο όταν επιλέχθηκε, ενώ η φυσική επιστροφή εξακολουθεί να εκτελείται ανά
Κιβώτιο.
...
αντίστοιχη επιλογή χρηστών.

Τα email αιτημάτων αποστέλλονται μόνο στα βασικά ορόσημα:

- δημιουργία/υποβολή κανονικού inbound ή return αιτήματος,
- δημιουργία staged manual/XLSX receipt που χρειάζεται έλεγχο,
- έγκριση,
- τελική ολοκλήρωση.

Αυτόματα ολοκληρωμένα synthetic receipts και ενδιάμεσες ενέργειες — όπως
σχόλια, αρχεία, ανάθεση, προετοιμασία, μεταφορά, hold, απόρριψη, ακύρωση,
overdue, quarantine και discrepancies — δεν παράγουν email. Εξακολουθούν όμως
να εμφανίζονται κανονικά στο καμπανάκι όταν προβλέπεται από τους υπάρχοντες
κανόνες παραληπτών. Τα επιτρεπόμενα email μπαίνουν σε ανθεκτική ουρά με
idempotency και έως πέντε προσπάθειες μεταφοράς. Πριν από κάθε προσπάθεια
ελέγχεται ξανά ότι ο παραλήπτης είναι ενεργός, έχει επιλέξει request email και
διαθέτει τρέχουσα διεύθυνση. Αποτυχία email δεν αναιρεί την επιχειρησιακή
ενέργεια.
...
Η έννοια **Boxes stuck** έχει καταργηθεί πλήρως από την ενεργή ανίχνευση.
Παλαιότερες τέτοιες εγγραφές δεν διαγράφονται: εμφανίζονται μόνο ως
**Retired legacy alert**, είναι επιλυμένες και δεν μπορούν να στείλουν
αυτόματο ή δοκιμαστικό email. Η κατάσταση και η κανονική ροή των Κιβωτίων δεν
αλλάζουν από αυτή την κατάργηση.
...
Κάθε νέο περιστατικό ειδοποίησης μπορεί να στείλει μόνο το αρχικό email
ανοίγματος. Αν η αποστολή αποτύχει, γίνονται έως πέντε συνολικές προσπάθειες.
Δεν αποστέλλονται υπενθυμίσεις, κλιμακώσεις ή email επίλυσης. Αν η συνθήκη
πάψει να ισχύει και εμφανιστεί ξανά αργότερα, δημιουργείται νέο περιστατικό που
μπορεί να στείλει ένα νέο αρχικό email.

Στη λεπτομέρεια εμφανίζεται το ιστορικό όλων των προσπαθειών. Παλαιά
`reminder`, `escalated` και `resolved` email παραμένουν ορατά με σαφή ένδειξη
**Legacy** μόνο για audit. Ο Admin μπορεί να στείλει ανεξάρτητο δοκιμαστικό
email για ενεργούς τύπους ειδοποίησης όταν έχει ρυθμιστεί το Microsoft Graph.
Το test δεν αλλάζει ούτε μηδενίζει τις προσπάθειες του αρχικού email.
...
Unassigned, ενεργό/αρχειοθετημένο File count και σύντομο summary των ενεργών
File references. Το compatibility Contents εξάγεται σε ξεχωριστή στήλη και
δεν πρέπει να συγχέεται με την αυθεντική λίστα Files.
...
### 10.5 Αναφορά φυσικών Files

Τα **Files CSV/XLSX** χρησιμοποιούν τα ίδια φίλτρα αναζήτησης, αποθήκης, Lot,
Παλέτας, Κιβωτίου, inherited status και activity με τη λίστα Files.
Περιλαμβάνουν File ID/reference, description, το αμετάβλητο barcode ως κείμενο,
θέση, ολόκληρη την
ιεραρχία Lot/Pallet/Box, inherited warehouse/status, active flag, archive
reason και ημερομηνίες. Μόνο Admin μπορεί να εξαγάγει Files από inherited
inactive hierarchy. Τα exports Κιβωτίων περιλαμβάνουν επίσης ενεργό και
αρχειοθετημένο File count και σύντομο File summary, ενώ τα Lot/Pallet exports
περιλαμβάνουν χωριστά active/archived File counts.
...
- ενεργοποιήσετε/απενεργοποιήσετε το ένα αρχικό email ανά περιστατικό
  αποθήκης,
- ενεργοποιήσετε/απενεργοποιήσετε τα email βασικών οροσήμων αιτημάτων,
...
- τους κύριους παραλήπτες του αρχικού email ανά ενεργή αποθήκη.

Χρησιμοποιήστε την για να επιβεβαιώσετε ότι η απενεργοποίηση email ενός χρήστη
δεν αφήνει αποθήκη χωρίς παραλήπτη. Δεν υπάρχει ξεχωριστή ομάδα κλιμάκωσης.

## 12. Ιστορικό και ακεραιότητα δεδομένων

Η εφαρμογή προστατεύει τις σχέσεις μεταξύ Lots, Παλετών, Κιβωτίων, φυσικών
Files, αιτημάτων και αποθηκών:
...
- τα File events κρατούν αυτοτελή before/after snapshots,
- τα request-item File snapshots παραμένουν αμετάβλητα ακόμη και μετά από
  αλλαγή του τρέχοντος File,
...
Η migration `0034_reduced_email_notifications` επιλύει τις ανοικτές παλιές
ειδοποιήσεις `box_stuck`, συμφιλιώνει τυχόν διπλές ανοικτές ειδοποιήσεις και
σημειώνει ως απορριφθέντα τα μη αποσταλμένα email αιτημάτων που δεν ανήκουν
πλέον στα επιτρεπόμενα ορόσημα. Δεν διαγράφει ιστορικές ειδοποιήσεις ή
καταγραφές αποστολής και δεν επιχειρεί να τις επαναφέρει σε downgrade.

Η migration `0035_first_class_box_files` προσθέτει τις εγγραφές
`box_files`, το append-only ιστορικό `box_file_events` και τα αμετάβλητα
`box_request_item_file_snapshots`. Για κάθε παλαιό Box και request item χωρίζει
το compatibility `contents` στο σύμβολο `|`, αγνοεί κενά τμήματα και διατηρεί
τη σειρά. Δημιουργεί deterministic references
`LEGACY-<BOX>-BOX-<box_id>-FILE-<n>` για tracked Files και
`LEGACY-REQUEST-ITEM-<request_item_id>-FILE-<n>` για ιστορικά request
snapshots. Τα παλαιά `boxes.contents` και `box_request_items.contents`
διατηρούνται ως compatibility columns· οι νέες λειτουργίες χρησιμοποιούν τα
δομημένα Files.

Πριν από παραγωγική αναβάθμιση σταματούν όλοι οι writers και λαμβάνεται κοινό
backup PostgreSQL/RustFS. Το read-only
`api/scripts/box_files_preflight.py` εκτελείται σε αναβαθμισμένο rehearsal
clone και ξανά αμέσως μετά το `alembic upgrade head`, πριν ανοίξουν τα writes·
πριν από τη migration χρησιμοποιούνται οι legacy SQL έλεγχοι του deployment
guide. Κατόπιν αναπτύσσονται μαζί API και web. Ο Admin
επαληθεύει επίσης το `GET /api/files/integrity`. Το downgrade της `0035`
επιτρέπεται μόνο όσο τα File tables ισούνται ακριβώς με το deterministic
legacy backfill και δεν υπάρχει κανένα File event· μετά από πραγματική χρήση
πρέπει να γίνει forward fix ή επαναφορά του ζευγαρωμένου backup. Αναλυτικές
εντολές και SQL έλεγχοι υπάρχουν στο
[First-class Box Files deployment](./FIRST_CLASS_BOX_FILES_DEPLOYMENT.md).
...
- Αν εμφανιστεί μήνυμα stale ή **Inventory changed**, μην επαναλάβετε τυφλά την
  ...
  την ανασκόπηση και αποδεχθείτε ξανά χωριστά τις μετακινήσεις Κιβωτίων και
  Files.
- Αν η ποσότητα διαφέρει, συμπληρώστε discrepancy reason.
- Ελέγξτε ότι κάθε File έχει reference, ότι δεν επαναλαμβάνεται σε άλλο
  Κιβώτιο του ίδιου Lot και ότι description/barcode δεν συγκρούονται μεταξύ
...
### Δεν βρίσκω ή δεν μπορώ να αλλάξω ένα File

- Ελέγξτε τα φίλτρα activity και inherited status στη σελίδα **Files**.
- Ελέγξτε ότι έχετε πρόσβαση στην τρέχουσα αποθήκη του Κιβωτίου.
- Viewer και Warehouse Mover έχουν μόνο ανάγνωση· αλλαγές κάνουν
  Operator/Admin.
- Reference που υπάρχει ήδη, ακόμη και σε άλλο Κιβώτιο του ίδιου Lot,
  απορρίπτεται ως διπλότυπο.
- File σε archived Κιβώτιο/Lot δεν μεταβάλλεται. Μόνο Admin βλέπει όλη την
  inherited inactive hierarchy.
- Αν υπάρχει ενεργή επιστροφή, αλλάξτε μόνο description/barcode ή διευθετήστε
  πρώτα την κράτηση. Force Admin ακυρώνει την κράτηση και απαιτεί αιτιολογία.
...
- Συμπληρώνετε Lot, προαιρετική Παλέτα και κάθε φυσικό File με μοναδικό
  reference· description/barcode είναι προαιρετικά metadata.
- Μετακινείτε τα Κιβώτια που αλλάζουν φυσική αποθήκη· η Παλέτα είναι
  οργανωτική και δεν μετακινείται.
- Μετακινείτε File μόνο σε Κιβώτιο του ίδιου Lot και ελέγχετε το File audit.
